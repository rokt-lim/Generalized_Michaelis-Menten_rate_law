# -*- coding: utf-8 -*-
"""
E. Rhythmic degradation of proteins, number of post-translational modifications = 2

dA(t)/dt   = g(t) - r_c*A_2(t)
dA_1(t)/dt = k_0*B*(A(t) - A_1(t) - A_2(t)) - k_1*B*A_1(t)
dA_2(t)/dt = k_1*B*A_1(t) - r_c*A_2(t)
...
###
def ODE_rhythmic_degradation_g_t_n2(t,y,pv):
    dy_dt = numpy.zeros(y.shape)
    #-- dA/dt = g(t) - r_c*A_2(t)
    #-- g(t) = g_max - 0.5*alpha_g*g_max - 0.5*alpha_g*g_max*cos(T_coeff*t - phi_g), phi_g = 0.0
    #-- dA_1/dt = k_0*B*(A - A_1 - A_2) - k_1*B*A_1
    #-- dA_2/dt = k_1*B*A_1 - r_c*A_2
    #-- parameters
    # pv[0] = g_max
    # pv[1] = alpha_g
    ...
    dy_dt = numpy.zeros(y.shape)
    dy_dt[0] = g_eval - pv[5]*y[2]
    dy_dt[1] = pv[7]*pv[4]*(y[0] - y[1] - y[2]) - pv[6]*pv[4]*y[1]
    dy_dt[2] = pv[6]*pv[4]*y[1] - pv[5]*y[2]
    #-- return
    return dy_dt
###

###
def solve_rhythmic_degradation_g_t_n2(t_start,t_end,dt,T_period,g_max,alpha_g,B_eval,r_c,k_1,k_0):
    #-- input
    ...
    # pv[6] = k_1
    # pv[7] = k_0
    #-- initial conditions for A, A_1, and A_2
    A_init = 1.0
    A_1_init = 0.0
    A_2_init = 0.0
    #-- solve the system of ODEs
    pv = [g_max,alpha_g,2.0*numpy.pi/T_period,0.0,B_eval,r_c,k_1,k_0]
    t_eval = numpy.linspace(t_start,t_end,int((t_end-t_start)/dt) + 1)
    sol = solve_ivp(fun=lambda t,y : ODE_rhythmic_degradation_g_t_n2(t,y,pv),\
        t_span=[t_start,t_end],y0=[A_init,A_1_init,A_2_init],method='LSODA',\
    ...
    df['r_t_gamma'] = r_c*A_2_gamma*df['A_inv'].values
    #-- save columns
    save_cols = ['t','A','B','A_1','A_2','R_t','r_t','r_t_Q','r_t_gamma']
    df = df.loc[:,save_cols]
    #-- return
    return df
###
...
###
def main_E_rhythmic_degradation_n2():
    #-- input
    #-- Constants for ODE: t_start, t_end, dt
    #-- T_period : period of g(t), unit = h
    #-- g_max    : maximum protein synthesis rate, unit = nM/h
    ...
    k_1 = 0.032196664
    k_0 = 0.016535068
    df_sol = solve_rhythmic_degradation_g_t_n2(t_start,t_end,dt,T_period,g_max,alpha_g,B_eval,r_c,k_1,k_0)
    df_sol.to_csv('test_rhythmic_degradation_n2.csv',index=False)
    #-- return
    return 0
###
...
main_E_rhythmic_degradation_n2()
